...
import { EntrepotService, Entrepot } from '../../services/entrepot.service';
...
export class Entrepots implements OnInit {
  ...
  openAddModal() { this.isEditMode = false; this.currentEntrepot = this.empty(); this.showModal = true; }
  openEditModal(e: Entrepot) { this.isEditMode = true; this.currentEntrepot = { ...e }; this.showModal = true; }
  closeModal() { this.showModal = false; }

  saveEntrepot() {
    if (this.isEditMode) {
      this.entrepotService.update(this.currentEntrepot.id, this.currentEntrepot).subscribe({
      ...
      this.entrepotService.create(this.currentEntrepot).subscribe({
        next: () => { this.charger(); this.closeModal(); },
        error: () => {
  ...
  confirmDelete(e: Entrepot) { this.entrepotToDelete = e; this.showDeleteConfirm = true; }
  cancelDelete() { this.entrepotToDelete = null; this.showDeleteConfirm = false; }

  deleteEntrepot() {
    if (this.entrepotToDelete) {
      this.entrepotService.delete(this.entrepotToDelete.id).subscribe({
        next: () => this.charger(),
        error: () => {
